...
public class KThread {
    ...
    if(status == statusFinished){
        ...
        //Alarm ala= new Alarm();
        papuQueue.waitForAccess(KThread.currentThread());
...
*/
    ...
    // Prueba hasta el communicator
     private static class PingTest implements Runnable { 
        ...
        public void run() { 
            for (int i=0; i<5; i++) {
                System.out.println("Soy Thread: "+ which);
                c1.speak(i);
                System.out.println("Thread "+which+" digo: "+ i); 
            }
        }
        private int which; 
        private Communicator c1; 
    }
    private static class PingTest1 implements Runnable { 
        PingTest1(int which, Communicator c) { 
        this.which = which; 
        c1=c; 
        } 
        public void run() { 
            for (int i=0; i<5; i++) {
                System.out.println("Soy thread: "+ which);
                System.out.println("Thread "+which+" escucho "+ c1.listen()); 
            }
        }
        private int which; 
        private Communicator c1; 
   ...
   public static void selfTest() { 
        ...
        cero = new KThread(new PingTest(0,c)).setName("forked thread0"); 
        cero.fork(); 
        uno = new KThread(new PingTest1(1,c)).setName("forked thread1"); 
        uno.fork();
        cero.join(); 
        uno.join();   
    } 
...
*/ 
    ...
    private ThreadQueue papuQueue = ThreadedKernel.scheduler.newThreadQueue(false);
